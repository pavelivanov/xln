import { cp, lstat, mkdir, mkdtemp, open, readFile, rename, rm, rmdir } from 'node:fs/promises';
...
const decodeState = (value: unknown): DeploymentCandidateState => {
  ...
  const rollbackReleaseId =
    rollbackValue === null ? null : releaseId(rollbackValue, 'DEPLOYMENT_CANDIDATE_ROLLBACK_ID_INVALID');
...
export const verifyDeploymentCandidateState = async (deploymentRoot: string): Promise<DeploymentCandidateSelection> => {
  await assertPlainDirectory(deploymentRoot, 'DEPLOYMENT_CANDIDATE_ROOT_INVALID');
...
const writeStateAtomically = async (deploymentRoot: string, state: DeploymentCandidateState): Promise<void> => {
...
export const activateDeploymentCandidate = async (
  ...
  return withActivationLock(deploymentRoot, async () => {
    ...
    if (expectedActiveReleaseId !== undefined && (current?.activeReleaseId ?? null) !== expectedActiveReleaseId) {
      throw new Error('DEPLOYMENT_CANDIDATE_ACTIVE_CHANGED');
    }
...
export const rollbackDeploymentCandidate = async (
  ...
  return withActivationLock(deploymentRoot, async () => {
    ...
    if (expectedActiveReleaseId !== undefined && current.state.activeReleaseId !== expectedActiveReleaseId) {
      throw new Error('DEPLOYMENT_CANDIDATE_ACTIVE_CHANGED');
    }
