...
import { EDGE_ROUTES, SURFACES, SURFACE_IDS } from './surfaces';
import { RELEASE_SCHEMA_VERSION, type CandidateReleaseManifest } from './manifest';
...
const walkRegularFiles = async (root: string, current = root): Promise<readonly string[]> => {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink())
      throw new Error(`CANDIDATE_RELEASE_SYMLINK:${toPortablePath(relative(root, pathname))}`);
...
const decodeGeneratedInputs = (
  ...
  const inputs = value.map(item => {
    ...
    exactKeys(
      item,
      ['id', 'owner', 'outputNamespace', 'definitionSha256', 'files'],
      'CANDIDATE_RELEASE_INPUT_KEYS_INVALID',
    );
...
const expectedApplications = (): CandidateReleaseManifest['applications'] =>
...
const decodeManifest = (value: unknown): CandidateReleaseManifest => {
  ...
  exactKeys(
    value,
    ['schemaVersion', 'releaseId', 'applications', 'generatedInputs', 'edgeRoutes', 'files'],
    'CANDIDATE_RELEASE_MANIFEST_KEYS_INVALID',
  );
...
): Promise<Readonly<{ raw: string; manifest: CandidateReleaseManifest }>> => {
  let raw: string;
  try {
...
const identityFor = (manifest: CandidateReleaseManifest): string =>
  hashBytes(
    Buffer.from(
      safeStringify({
...
export const verifyCandidateReleaseDirectory = async (releaseDirectory: string): Promise<CandidateReleaseManifest> => {
  ...
  if (safeStringify(actualPaths) !== safeStringify(expectedPaths))
    throw new Error('CANDIDATE_RELEASE_FILE_SET_MISMATCH');
