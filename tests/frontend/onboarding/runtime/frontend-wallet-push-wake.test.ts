...
describe('React wallet push-wake controls', () => {
  ...
  test('projects only redacted registration state into React', () => {
    const projection = read('frontend/packages/browser/src/wallet/wallet-push-wake.ts');
    const react = [
      read('frontend/apps/wallet/src/push-wake/wallet-push-wake.tsx'),
      read('frontend/apps/wallet/src/push-wake/wallet-push-wake-source.ts'),
    ].join('\n');
  ...
  test('keeps enable and disable controls adjacent to saved recovery services', () => {
    const recovery = read('frontend/apps/wallet/src/recovery/wallet-recovery-services.tsx');
    const push = read('frontend/apps/wallet/src/push-wake/wallet-push-wake.tsx');
