...
} from '../../../../frontend/apps/wallet/src/settings/wallet-settings-model';
...
describe('React wallet browser preferences', () => {
  ...
  test('keeps secrets and Runtime state outside the preference surface', () => {
    const model = readFileSync('frontend/apps/wallet/src/settings/wallet-settings-model.ts', 'utf8');
    const view = readFileSync('frontend/apps/wallet/src/settings/wallet-settings.tsx', 'utf8');
    const source = `${model}\n${view}`;
