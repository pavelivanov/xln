...
describe('React wallet Brain Vault derivation boundary', () => {
  test('uses the canonical versioned browser worker and validated shard protocol', () => {
    const runtime = read('frontend/bridges/wallet/brainvault/wallet-brainvault-worker-runtime.ts');
    const derivation = read('frontend/bridges/wallet/brainvault/wallet-brainvault-browser-derivation.ts');
    const finalization = read('frontend/bridges/wallet/brainvault/wallet-brainvault-material-finalization.ts');
