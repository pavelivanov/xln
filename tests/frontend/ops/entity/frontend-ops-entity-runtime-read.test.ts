...
} from '../../../../frontend/apps/ops/src/entity-workspace/ops-entity-workspace-source';
import {
...
} from '../../../../frontend/packages/runtime-client/src/entity/profile/entity-workspace-hub-policy';
import {
...
describe('React Entity workspace Runtime read boundary', () => {
  ...
  test('loads only on the workspace route and exposes bounded reads plus full cleanup', async () => {
    const [command, main, page, runtime, source, projection] = await Promise.all([
      ...
      Bun.file('frontend/apps/ops/src/entity-workspace/ops-entity-workspace-runtime.ts').text(),
  ...
  test('validates Account page navigation before issuing another bounded read', async () => {
    ...
    const implementation = await Bun.file('frontend/apps/ops/src/entity-workspace/ops-entity-workspace-source.ts').text();
