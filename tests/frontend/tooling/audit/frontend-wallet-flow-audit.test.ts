...
describe('React wallet WP6 flow audit', () => {
  ...
  test('accounts for every named WP6 requirement without inventing completion', () => {
    ...
    expect(WALLET_REQUIREMENT_AUDIT.every(({ disposition }) => disposition === 'implemented')).toBe(true);
    expect(WALLET_REQUIREMENT_AUDIT.find(({ id }) => id === 'lending')).toMatchObject({
      disposition: 'implemented', evidenceId: 'account-rail-and-entity-selection',
    });
  });
