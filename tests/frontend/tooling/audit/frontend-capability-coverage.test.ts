...
describe('frontend capability inventory', () => {
  ...
  test('records the implemented site capability without claiming production cutover', () => {
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-recovery')?.currentSources)
      .toContain('frontend/bridges/wallet/canonical/wallet-canonical-push-wake.ts');
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-browser-lifecycle')?.status).toBe('implemented');
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-runtime-discovery')?.status).toBe('implemented');
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-finance')?.status).toBe('implemented');
    ...
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-payments-and-markets')?.behavior)
      .toContain('React-owned orderbooks and same-j commands with exact cross-j submission/cancellation finality and lossless Activity paging');
    expect(CAPABILITIES.find(({ id }) => id === 'wallet-payments-and-markets')?.currentSources)
      .toContain('frontend/bridges/wallet/canonical/wallet-canonical-external-provider.ts');
  ...
  test('records implemented Health, QA and HLT with the accepted relay-detail boundary', () => {
    ...
    expect(opsHealth?.behavior).toContain(
      'Endpoint-bounded relay inventory with counts, managed/external Runtime IDs, subscriptions, and truthful unavailable per-client age, last-seen, and topics',
    );
    expect(opsHealth?.behavior).toContain(
