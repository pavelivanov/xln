...
describe('scoped frontend checks', () => {
  test('slice builds the selected app and runs exactly its registered browser spec', () => {
    const commands = createCheckCommands(parseCheckRequest(['--surface=ops', '--level=slice', '--spec=tests/react-candidate/ops-workspace-session.spec.ts']));
    ...
    expect(commands.at(-1)?.argv.at(-1)).toBe('tests/react-candidate/ops-workspace-session.spec.ts');
