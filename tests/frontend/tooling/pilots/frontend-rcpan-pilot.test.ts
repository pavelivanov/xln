...
import {
  ...
  RCPAN_SYSTEMS,
  RCPAN_UPGRADES,
} from '../../../../frontend/apps/site/src/rcpan/rcpan-content';
...
describe('React RCPAN pilot', () => {
  ...
  test('consumes the canonical microscope model instead of reproducing account math', async () => {
    const page = await readFile(new URL('frontend/apps/site/src/rcpan/rcpan-page.tsx', REPOSITORY_ROOT), 'utf8');
    expect(page).toContain("from '../../../../packages/ui/src/rcpan/microscope/microscope-model'");
