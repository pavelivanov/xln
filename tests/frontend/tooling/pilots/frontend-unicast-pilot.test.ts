...
import {
  ...
  parseNetworkTps,
} from '../../../../frontend/apps/site/src/unicast/unicast-model';
...
describe('React unicast pilot', () => {
  ...
  test('uses a deterministic visualization instead of random node placement', async () => {
    const model = await readFile(new URL('frontend/apps/site/src/unicast/unicast-model.ts', REPOSITORY_ROOT), 'utf8');
