...
} from '../../../../../frontend/packages/browser/src/runtime/wallet-boot-lifecycle';
...
describe('browser wallet boot lifecycle', () => {
  ...
  test('does not start a local Runtime while its restored vault remains locked', async () => {
    const harness = createBootHarness();
    const dependencies = {
      ...harness.dependencies,
      validateLocalVault: () => false,
    };

    expect(await runWalletBootLifecycle(dependencies)).toBe('cancelled');
    expect(harness.calls).toContain('vault');
    expect(harness.calls).not.toContain('runtime');
  });
  ...
  test('keeps the canonical app shell on the shared lifecycle boundary', () => {
    ...
    expect(shell).toContain('initializeEmbeddedRuntimeOnce');
    expect(shell).toContain('startWalletEmbeddedRuntime()');
    expect(shell).toContain('subscribeWalletEmbeddedRuntime');
    expect(runtime).toContain('export const startWalletEmbeddedRuntime = startBrowserRuntime');
    expect(runtime).toContain('export const stopWalletEmbeddedRuntime');
  });
});
