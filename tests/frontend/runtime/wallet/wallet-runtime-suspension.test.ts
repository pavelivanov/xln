...
import { suspendWalletRuntimeActivity } from '../../../../frontend/packages/browser/src/runtime/wallet-runtime-suspension';
...
describe('wallet Runtime suspension boundary', () => {
  test('pauses watchers, drains accepted work, then fences persistence and stops transport', async () => {
    const target = createTarget();
    const events: string[] = [];
    await suspendWalletRuntimeActivity(target, {
      stopWatchers: async () => { events.push(`watchers:${target.infrastructure.jurisdictionWatchersPaused}`); },
      waitForWorkDrained: async (_target, timeout) => {
        events.push(`work:${target.infrastructure.persistenceQuiescing}:${timeout}`);
        return true;
      },
    ...
    expect(events).toEqual(['watchers:true', 'work:false:31', 'loop:true:31', 'p2p:17']);
