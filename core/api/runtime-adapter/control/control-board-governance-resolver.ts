...
import type { ControlBoardGovernanceRequest } from './control-board-governance';
...
export const projectControlTakeoverTargets = (
  input: Readonly<{
  ...
  for (const candidate of input.candidates) {
    ...
    if (
      ...
      normalize(candidate.signerId) !== currentSignerId ||
      !currentProvider ||
      normalize(candidate.entityProviderAddress) !== currentProvider ||
      !candidate.validators.some(validator => normalize(validator) === currentSignerId)
    )
      continue;
...
const captureTarget = async (
  env: RuntimeReplica,
  request: Exclude<ControlBoardGovernanceRequest, { operation: 'targets' }>,
) =>
  withRuntimeCommittedRead(env, () => {
    const target = projectTargets(env, request.shareholderEntityId, request.signerId).find(
...
export const resolveControlBoardGovernance = async (
  ...
  if (request.operation === 'targets') {
    return withRuntimeCommittedRead(env, () => ({
      targets: projectTargets(env, request.shareholderEntityId, request.signerId),
    }));
  }
  ...
  const review = {
    targetEntityId: captured.target.entityId,
    targetName: captured.target.name,
    newBoardHash: captured.newBoardHash,
    actionNonce: captured.status.actionNonce + 1n,
  };
  if (request.operation === 'review') {
    if (captured.status.proposedBoardHash !== ZERO_HASH) throw new Error('CONTROL_TAKEOVER_PROPOSAL_ALREADY_PENDING');
    if (captured.status.currentBoardHash === captured.newBoardHash)
      throw new Error('CONTROL_TAKEOVER_BOARD_ALREADY_ACTIVE');
    return review;
  }
  let expectedActionNonce: bigint;
  try {
    expectedActionNonce = BigInt(request.expectedActionNonce);
  } catch {
    throw new Error(`CONTROL_TAKEOVER_ACTION_NONCE_INVALID:${request.expectedActionNonce}`);
  }
  if (request.operation === 'activation-prepare') {
    const activation = requireActivationReview(captured);
    if (
      normalize(request.expectedBoardHash) !== activation.newBoardHash ||
      expectedActionNonce !== activation.actionNonce
    ) {
