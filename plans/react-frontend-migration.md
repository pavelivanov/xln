...
sent through the canonical persisted Activity adapter. Timeframe reads retain
the adapter's existing 1,000-frame maximum; strict projection checks the echoed
range, scan bound, and every returned event against inclusive From/To bounds.
Invalid reversed ranges remain disabled and visible, Clear removes the range
without changing mode, and mode changes reset only the certified page cursor.
The same request options drive live and exact Time Machine reads without a
second history source, write path, wall clock, or financial derivation.

Focused Entity workspace coverage passes 115 / 115 tests with 798 expectations,
and the narrower Activity boundary passes 15 / 15 with 126 expectations. The
all-surface React-local gate scans 695 files with zero unsafe findings. All four
React production surfaces build; site/docs/wallet/ops transform 401 / 35 /
1,860 / 1,642 modules, and the ops workspace CSS/view/Runtime chunks are 39.25 /
40.45 / 45.95 kB. The canonical frontend check reports zero errors/warnings
while transforming 4,681 SSR plus 6,432 client modules. The complete ops
candidate matrix passes 30 / 30 across the required viewports.

The final isolated real-H1 flow passes in 33.5 seconds (34.8 seconds total)
with candidate `77fa520db3bb`. It proves invalid-range prevention, applies an
inclusive range over exact persisted event timestamps, clears it while keeping
Timeframe active, returns to Pagination, and completes the existing controls.
Strict browser health emits no console error. Mobile 390x844, laptop 1366x900,
and wide 1920x1080 screenshots were inspected with aligned native date inputs,
compact controls, bounded list height, and no clipping or horizontal overflow.
The 224-file frontend suite reports 1,336 passes, the unchanged 16-failure plus
one-error repository baseline, and 7,678 expectations across 1,352 tests; no
slice-owned test fails.
...
The following Activity presentation slice restores the legacy event-tone
precedence from fields already validated by the React adapter boundary. A
small pure UI projection maps errors before on-chain evidence, then inbound
and outbound payments, cross-j swaps, swaps, and neutral activity. Rows expose
the exact derived tone alongside kind, type, direction, and status, while a
restrained rail, row index, and direction label carry the visual distinction.
No Runtime state, financial formula, token interpretation, or alternate
activity source is introduced.

Focused Activity coverage passes 18 / 18 tests with 158 expectations; the
complete focused Entity workspace batch passes 71 / 71 with 533 expectations.
The all-surface React-local gate scans 697 files with zero unsafe findings. All
four React production surfaces build; site/docs/wallet/ops transform 401 / 35 /
1,860 / 1,644 modules, and the ops workspace CSS/view/Runtime chunks are 40.26 /
42.19 / 49.23 kB. The canonical frontend check reports zero errors/warnings
while transforming 4,681 SSR plus 6,432 client modules. The complete ops
candidate matrix passes 30 / 30 across the required viewports.

The isolated real-H1 flow passes in 43.2 seconds (44.7 seconds total) with
candidate `b092aebc1782`. It verifies every rendered row tone against its exact
kind, type, direction, and status while completing the existing Activity and
Time Machine flow. Strict browser health emits no console error. Dedicated
mobile 390x844, laptop 1366x900, and wide 1920x1080 screenshots were inspected
with legible semantic rails and labels, stable alignment, and no clipping or
horizontal overflow. The 224-file frontend suite reports 1,340 passes, the
unchanged 16-failure plus one-error repository baseline, and 7,713 expectations
across 1,356 tests; no slice-owned test fails.
...
The following Account evidence slice projects exact nonfinancial protocol
context already committed in each Account state: chain and depository domain,
last finalized Jurisdiction height, Jurisdiction nonce, and both dispute
response windows. The adapter validates every field before publication, and
the read-only rail keeps long addresses available as exact title evidence.
No delta, balance, financial formula, command path, or Account transition is
read or changed.

Focused Account evidence coverage passes 11 / 11 tests with 40 expectations;
the wider focused Entity workspace batch passes 80 / 80 with 416 expectations.
The all-surface React-local gate scans 697 files with zero unsafe findings. All
four React production surfaces build; site/docs/wallet/ops transform 401 / 35 /
1,860 / 1,644 modules, and the ops workspace CSS/view/Runtime chunks are 40.61 /
43.42 / 50.11 kB. The canonical frontend check reports zero errors/warnings
while transforming 4,681 SSR plus 6,432 client modules. The complete ops
candidate matrix passes 30 / 30 across the required viewports.

The isolated real-H1 flow passes in 32.4 seconds (33.8 seconds total) with
candidate `4c150e289e4b`. It verifies exact chain, depository, finalized
Jurisdiction height, nonce, and response-window evidence while completing the
existing Activity and Time Machine flow. Strict browser health emits no
console error. Dedicated mobile 390x844, laptop 1366x900, and wide 1920x1080
screenshots were inspected with readable protocol labels, intentional address
truncation, and no horizontal overflow. Screenshot review exposed a cramped
three-column laptop row; the final candidate uses a two-column evidence grid
at intermediate desktop widths and the regenerated H1 capture is clean.

The 224-file frontend suite reports 1,343 passes, the unchanged 16-failure plus
one-error repository baseline, and 7,733 expectations across 1,359 tests; no
slice-owned test fails.
...
The following Consensus evidence slice projects the exact committed Entity
frame coordinates already present in the bounded remote Runtime frame: Entity
height, deterministic UTC timestamp, certified frame head, and last finalized
Jurisdiction height. Non-genesis Entity state must expose the certified head
that parents its next frame; malformed or missing lineage now fails at the
React adapter boundary. Validator-local proposals, locks, votes, and
certificates remain deliberately absent.

Focused projection and wiring coverage passes 14 / 14 tests with 118
expectations; the wider focused Entity workspace batch passes 82 / 82 with 427
expectations after two historical-frame fixtures were brought up to the same
committed-head invariant. The all-surface React-local gate scans 697 files with
zero unsafe findings. All four React production surfaces build; site/docs/
wallet/ops transform 401 / 35 / 1,860 / 1,644 modules, and the ops workspace
CSS/view/Runtime chunks are 40.72 / 44.30 / 51.21 kB. The canonical frontend
check reports zero errors/warnings while transforming 4,681 SSR plus 6,432
client modules. The complete ops candidate matrix passes 30 / 30 across the
required viewports.

The isolated real-H1 flow passes in 33.1 seconds (34.4 seconds total) with
candidate `2aa9d0f01955`. It verifies exact Entity height, finalized
Jurisdiction height, UTC timestamp plus ISO instant, and a nonempty certified
frame head while completing the existing Account, Activity, and Time Machine
flow. Strict browser health emits no console error. Dedicated mobile 390x844,
laptop 1366x900, and wide 1920x1080 Consensus screenshots were inspected with
readable evidence, stable two-column alignment, and no clipping or horizontal
overflow.

The 224-file frontend suite reports 1,345 passes, the unchanged 16-failure plus
one-error repository baseline, and 7,744 expectations across 1,361 tests; no
slice-owned test fails.
