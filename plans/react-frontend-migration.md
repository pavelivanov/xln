...
| O09 open | Port Stack Manager list/inspect/select from `StackManager.svelte`. | Actual configured stacks load, selection is shared and API failures remain visible. `ops-workspace-settings.spec.ts`, `tests/frontend/settings/stack-manager-client.test.ts`. |
