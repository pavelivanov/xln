...
export const suspendWalletRuntimeActivity = async <Target extends WalletRuntimeSuspensionTarget>(
  ...
  // Stop new chain observations while keeping peer replies admissible until
  // already accepted financial work and its acknowledgements have drained.
  const previousWatcherPause = target.infrastructure?.jurisdictionWatchersPaused;
  if (target.infrastructure) target.infrastructure.jurisdictionWatchersPaused = true;
  ...
  if (target.infrastructure) {
    target.infrastructure.persistenceQuiescing = true;
    if (previousWatcherPause === undefined) delete target.infrastructure.jurisdictionWatchersPaused;
    else target.infrastructure.jurisdictionWatchersPaused = previousWatcherPause;
    target.infrastructure.persistencePaused = true;
  }
