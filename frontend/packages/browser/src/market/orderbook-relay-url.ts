...
const isTrustedBrowserRelayUrl = (parsed: URL, pageLocation: BrowserLocationLike): boolean => {
  const pageHost = String(pageLocation.hostname || '')
    .trim()
    .toLowerCase();
  const relayHost = String(parsed.hostname || '')
    .trim()
    .toLowerCase();
...
export const resolveOrderbookRelayWsUrl = (
  ...
  try {
    const parsed = trimmed.startsWith('/') ? new URL(trimmed, pageLocation.origin) : new URL(trimmed);
