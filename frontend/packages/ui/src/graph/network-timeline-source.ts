...
import { buildRuntimeActivityEvents } from '@xln/core/api/public/activity-history';
...
} from '@xln/core/storage/keys';
import { normalizeRuntimeTimelineIndex, type RuntimeTimelineIndex } from '../../../runtime-client/src/graph/runtime-graph-timeline';
import { decodeNetworkMachineCue, type NetworkMachineCue } from '../../../runtime-client/src/scenario/network-machine';
import { isUnknownRecord, rejectExtraKeys } from '../../../runtime-client/src/boundary';
...
const materializedGraphHead = (height: number): RuntimeAdapterGraphFrame['head'] => ({
  schemaVersion: STORAGE_SCHEMA_VERSION,
  latestHeight: height,
  latestMaterializedHeight: height,
  latestSnapshotHeight: 0,
  snapshotPeriodFrames: DEFAULT_SNAPSHOT_PERIOD_FRAMES,
...
export type NetworkTimelineSource = {
  readonly kind: 'adapter' | 'scenario' | 'trail';
  readonly runtimeId: string;
  readIndex(): Promise<RuntimeTimelineIndex>;
  /** Only an in-memory scenario owns full snapshots; portable graph trails do not. */
  readSnapshot?: (height: number) => EnvSnapshot;
  readSolvency?: (height: number) => Promise<RuntimeAdapterSolvencySummary>;
...
export const adapterNetworkTimelineSource = (
  ...
  return {
    kind: 'adapter',
    runtimeId: expected,
    readSolvency: height => adapter.read<RuntimeAdapterSolvencySummary>('solvency-summary', { atHeight: height }),
...
export const graphFrameFromSnapshot = (
  ...
  return {
    // A recording is fully materialized in memory and has no persisted
    // checkpoints or retained WAL bytes. Keep the same in-memory head contract
    // as the Runtime adapter so its exported frames pass the trail decoder.
    head: materializedGraphHead(height),
    runtimeId,
    height,
    timestamp,
    stateHash: '',
...
export const scenarioNetworkTimelineSource = (
  ...
  projectSolvency?: (snapshot: EnvSnapshot) => RuntimeAdapterSolvencySummary,
): NetworkTimelineSource => {
  const expected = normalizeId(runtimeId);
  if (!expected) throw new Error('NETWORK_TIMELINE_RUNTIME_ID_REQUIRED');
  ...
  return {
    ...
    readSnapshot: snapshotAt,
    ...(projectSolvency ? { readSolvency: async (height: number) => projectSolvency(snapshotAt(height)) } : {}),
...
export const parseNetworkTrail = (text: string): NetworkTrail => {
  ...
  for (const [height, frame] of Object.entries(parsed['frames'])) {
    ...
    // The canonical scenario recorder intentionally omits storage-only head data.
    // Materialize the adapter contract at this UI boundary before validation.
    const normalizedFrame = isUnknownRecord(frame) && frame['head'] === undefined
      ? { ...frame, head: materializedGraphHead(numericHeight) }
      : frame;
    if (!/^\d+$/.test(height) || !isRuntimeAdapterGraphFrame(normalizedFrame, runtimeId, numericHeight)) {
      throw new Error(`NETWORK_TRAIL_FRAME_INVALID:${height}`);
    }
    frames[height] = normalizedFrame;
  }
  ...
  const cues = parsed['cues'] === undefined
    ? undefined
    : Array.isArray(parsed['cues'])
      ? parsed['cues'].map(decodeNetworkMachineCue)
      : (() => { throw new Error('NETWORK_TRAIL_CUES_INVALID'); })();
