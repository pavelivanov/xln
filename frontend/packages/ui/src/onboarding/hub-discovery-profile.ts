...
} from '../../../runtime-client/src/entity/account-action-txs';

import {
  hasUsableOpenAccountCounterpartyProfile,
  waitForCounterpartyRuntimeRoutes,
} from '../../../runtime-client/src/runtime/p2p-prefetch';
import {
  isCommittedAccount,
  isOpeningAccount,
  normalizeEntityId,
} from '../../../runtime-client/src/entity/entity-replica';

const isAccountsMapLike = (value: unknown): value is ReadonlyMap<string, AccountReadView> => isMapLike(value);

export type HubDiscoveryRuntime = Readonly<{
  id: string;
  type: 'local' | 'remote';
  label: string;
  wsUrl?: string;
  hubEntities?: ReadonlyArray<Readonly<{
    entityId: string;
    label?: string;
...
export function buildHubDiscoveryRemoteHubsFromRuntimes(
  runtimes: Iterable<HubDiscoveryRuntime>,
