import type { NavigationSelection } from './navigation-selection';
import type { Runtime as VaultRuntime } from '../../../../bridges/vault/recovery/vault-recovery';
...
const runtimeMatchesView = (
  runtimeId: string,
  runtimeView: RuntimeNavigationViewProjection | null | undefined,
): boolean => normalizeId(runtimeId) === normalizeId(runtimeView?.runtimeId);
...
export function buildHierarchicalNavigationView(
  ...
  const selectedRuntime = Array.from(runtimes.values()).find(
    runtime => normalizeId(runtime.id) === normalizeId(selectedRuntimeId),
  );
  const usesActiveVault =
    selectedRuntime?.type !== 'remote' && normalizeId(activeVaultRuntime?.id) === normalizeId(selectedRuntimeId);
  ...
  const accountItems: NavigationItem[] =
    selectedRuntimeHasProjection && active && selectedEntityId && activeEntityId === selectedEntityId
