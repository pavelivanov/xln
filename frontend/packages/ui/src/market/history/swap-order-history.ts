import { amountToUsd } from '../../../../runtime-client/src/asset-pricing';
import { requireTokenDecimals } from '../../../../runtime-client/src/token-metadata';
import type { SwapBookEntry } from '@xln/core/api/public/runtime-module';
import { toBigIntSafe } from '../format/swap-formatting';
import {
  requireExactKeys as requireExactKeysWithOptional,
  requireUnknownRecord as requireRecord,
...
const decodeResolveRecord = (value: unknown, index: number): ResolveRecord => {
  ...
  return {
    ...
    fillNumerator:
      fillNumerator === null
        ? null
        : requireBigInt(fillNumerator, `SWAP_HISTORY_RESOLVE_FILL_NUMERATOR_INVALID:${index}`),
    fillDenominator:
      fillDenominator === null
        ? null
        : requireBigInt(fillDenominator, `SWAP_HISTORY_RESOLVE_FILL_DENOMINATOR_INVALID:${index}`),
    ...
    executionGiveAmount:
      executionGiveAmount === null
        ? null
        : requireBigInt(executionGiveAmount, `SWAP_HISTORY_RESOLVE_GIVE_INVALID:${index}`),
    executionWantAmount:
      executionWantAmount === null
        ? null
        : requireBigInt(executionWantAmount, `SWAP_HISTORY_RESOLVE_WANT_INVALID:${index}`),
    feeTokenId:
      feeTokenId === null
        ? null
        : requireUint(feeTokenId, `SWAP_HISTORY_RESOLVE_FEE_TOKEN_INVALID:${index}`, 0xffffffff),
...
const decodeLifecycle = (value: unknown, index: number): SwapHistoryLifecycle => {
  ...
  return {
    ...
    liveGiveAmount:
      record['liveGiveAmount'] === null
        ? null
        : requireBigInt(record['liveGiveAmount'], `SWAP_HISTORY_LIVE_GIVE_INVALID:${index}`),
    liveWantAmount:
      record['liveWantAmount'] === null
        ? null
        : requireBigInt(record['liveWantAmount'], `SWAP_HISTORY_LIVE_WANT_INVALID:${index}`),
...
export const decodeSwapHistoryPage = (value: unknown): SwapHistoryPage => {
  ...
  requireExactKeys(
    page,
    ['entityId', 'accountId', 'latestHeight', 'items', 'nextCursor'],
    'SWAP_HISTORY_PAGE_FIELDS_INVALID',
  );
...
export function buildClosedOrderViews(
  ...
  return (Array.isArray(closedOfferLifecycles) ? closedOfferLifecycles : [])
    .map(offer => {
      ...
      const filledPpm = summary.filledPpm;
      const filledPercent =
        filledPpm >= deps.filledDisplayPpmThreshold ? 100 : Number((filledPpm * 10_000n) / 1_000_000n) / 100;
