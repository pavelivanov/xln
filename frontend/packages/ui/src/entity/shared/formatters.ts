 ...
 */
export function formatTokenAmount(_tokenId: number, amount: bigint, decimals: number): string {
...
export function parseAmount(str: string, decimals: number): bigint {
  ...
  try {
    ...
    if (fractionalPart.length > decimals) {
      throw new Error(`Too many decimal places: ${parts[1]?.length || 0} (max: ${decimals})`);
    }
