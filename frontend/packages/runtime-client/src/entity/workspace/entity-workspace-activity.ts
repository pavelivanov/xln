...
const requireActivityCursor = (value: unknown, code: string): string | null =>
  value === null || value === undefined ? null : nonemptyText(value, code);
...
const requireActivityQuery = (input: EntityWorkspaceActivityQueryOptions) => {
  ...
  return {
    cursor: requireActivityCursor(input.cursor, 'ENTITY_WORKSPACE_ACTIVITY_CURSOR_INVALID'),
...
export const buildEntityWorkspaceActivityQuery = (
  ...
  const request = requireActivityQuery(input);
  return {
    ...(request.cursor === null ? { beforeHeight: context.height } : { cursor: request.cursor }),
...
export function projectEntityWorkspaceActivity(input: Readonly<{
  ...
  requireFilters(
    page['filters'], context, request.kind, request.types, request.query,
    request.timeframe.fromTimestamp, request.timeframe.toTimestamp,
  );
  const returnedCursor = requireActivityCursor(
    page['cursor'],
    'ENTITY_WORKSPACE_ACTIVITY_CURSOR_INVALID',
  );
  if (returnedCursor !== request.cursor) throw new Error('ENTITY_WORKSPACE_ACTIVITY_CURSOR_MISMATCH');
  ...
  if (latestHeight === 0) {
    if (fromHeight !== 0 || toHeight !== 0 || scannedFrames !== 0 || returned !== 0
      || requireActivityCursor(page['nextCursor'], 'ENTITY_WORKSPACE_ACTIVITY_NEXT_CURSOR_INVALID') !== null) {
    ...
    return {
      ...
      types: request.types, requestedCursor: request.cursor,
      isLatestPage: request.cursor === null, latestHeight,
      fromHeight, toHeight, scannedFrames, nextCursor: null, events: [],
    };
  }

  const maximumToHeight = Math.min(latestHeight, context.height);
  if (fromHeight < 1 || fromHeight > toHeight || toHeight > maximumToHeight
    || (request.cursor === null && toHeight !== maximumToHeight)) {
  ...
  return {
    ...
    nextCursor: requireActivityCursor(page['nextCursor'], 'ENTITY_WORKSPACE_ACTIVITY_NEXT_CURSOR_INVALID'),
    events,
  };
}
...
export function appendEntityWorkspaceActivityPage(
  ...
  if (previous.nextCursor === null || next.requestedCursor !== previous.nextCursor
    || (next.toHeight !== previous.fromHeight && next.toHeight !== previous.fromHeight - 1)
    || next.latestHeight !== previous.latestHeight) {
