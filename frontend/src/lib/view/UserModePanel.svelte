<script lang="ts">
  ...
  const formationRuntimeProjection = $derived.by((): FormationRuntimeProjection => buildFormationRuntimeProjection(currentFrame));

  const onboardingRuntimeProjection = $derived.by(() => buildOnboardingRuntimeProjection({
    currentFrame,
    selectedEntityId,
    selectedSignerId,
    selectedReplica,
    selectedReplicaJurisdiction,
    activeJurisdictionName: getFrameActiveJurisdiction(currentFrame),
    runtimeSigners: $activeRuntimeStore?.signers || [],
  }));
