...
test('built routes, lazy assets, worker URLs and shared-origin storage retain exact release identity', async ({
  page,
}, testInfo) => {
...
test('built Wallet cancels and commits an exact Ownership release through same-origin Runtime WebSocket', async ({
  page,
}, testInfo) => {
  const before = await identity(page);
  const errors = observeBrowserErrors(page);
  ...
  const response = await page.request.post(`${fixtureOrigin}/ownership-release-fixture?slot=${testInfo.project.name}`);
  expect(response.ok()).toBe(true);
...
test('built workspace records exact graph history and restores the connected Runtime', async ({ page }, testInfo) => {
  const before = await identity(page);
  const errors = observeBrowserErrors(page);
  const fixture = await selectSameOriginRuntime(page);
