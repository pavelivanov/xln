...
test(
  ...
  async ({ page }, testInfo) => {
    ...
    await panel.getByLabel('Name', { exact: true }).fill(BRAINVAULT_NODE_OWNER_NAME);
    await panel.getByLabel('Passphrase', { exact: true }).fill(BRAINVAULT_NODE_OWNER_PASSPHRASE);
    await panel.getByRole('button', { name: 'Derive and install on selected node', exact: true }).click();
    ...
    expect(await readStorage(page)).not.toContain(BRAINVAULT_NODE_OWNER_PASSPHRASE);
