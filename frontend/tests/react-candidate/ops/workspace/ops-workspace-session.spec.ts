...
test(
  ...
  async ({ page }, testInfo) => {
    ...
    // Historical reads replay the persisted WAL; the canonical remote adapter
    // allows 15 seconds. The default five-second UI assertion races that read.
    await expect(peer.getByTestId('time-machine-mode')).toHaveText(`History · h${latest - 1}`, { timeout: 15_000 });
    await expect(input).toBeEnabled();
    await expect(input).toHaveValue(String(latest - 1));
    await expect(peer.getByTestId('entity-workspace-shell')).toHaveAttribute('data-read-status', 'ready');
