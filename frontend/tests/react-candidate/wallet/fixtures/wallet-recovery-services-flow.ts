...
export async function runWalletRecoveryServicesFlow(page: Page, testInfo: TestInfo): Promise<void> {
  ...
  await expect(page.getByRole('list', { name: 'Recovery service status' })).toContainText(fixture.recovery.towerUrl);
  ...
  const unsavedUrl = `${fixture.recovery.towerUrl}/unsaved`;
  ...
  const officialStatus = serviceStatus
    .locator('li')
    .filter({ has: page.getByText(fixture.recovery.towerUrl, { exact: true }) });
