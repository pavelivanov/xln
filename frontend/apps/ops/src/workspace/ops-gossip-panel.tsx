...
import { useOpenWorkspaceEntity } from './ops-workspace-navigation';
...
export function OpsGossipPanel() {
  ...
  const recorded = useMemo(() => {
    if (!network.selectedStep) return null;
    const frame = network.frames.get(network.selectedStep.activeRuntimeId);
    if (!frame) return null;
    return { runtimeId: frame.runtimeId, height: frame.height,
      directory: buildGossipDirectoryViewFromRuntimeEntities({ entities: frame.entities.map(entity => entity.summary), runtimeId: frame.runtimeId }) };
  }, [network]);
  const data = network.selectedStep ? recorded : snapshot.data;
