...
import type { OpsWorkspaceQueryClient } from './ops-workspace-query';
...
export function OpsEntityAuditPanel() {
  ...
  return <section className="ops-evidence-panel" data-testid="entity-audit-panel">
    <header><div><small>Reference Entity audit</small><h2>{state.context.entityName ?? 'Entity Audit'}</h2></div><button disabled={!entityId || (!recorded && snapshot.loading)} onClick={() => { if (recorded) void opsEntityWorkspaceSource.refresh(); else if (refresh) void refresh(); }} type="button">Refresh</button></header>
