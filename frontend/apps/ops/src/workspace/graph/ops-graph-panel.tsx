...
import { mergeRuntimeGraphProjections, projectRuntimeGraphFrame, projectRuntimeEnv } from '../../../../../packages/ui/src/graph/runtime-graph-projection';
...
import { runtimeGraphCanonicity, runtimeGraphControlOperations } from '../../../../../packages/browser/src/graph/runtime-graph-control-store';
...
import { collectGraphTokenIds } from '../../../../../packages/ui/src/graph/three/interaction/graph3d-actions';
...
import { projectOpsGraphEffects } from './ops-graph-effects';
...
export function OpsGraphPanel() {
  ...
  const scene = useRef<Awaited<ReturnType<typeof createOpsGraphScene>> | null>(null);
  ...
  const [xr, setXr] = useState({ supported: false, active: false });
  ...
  const effectSignals = useMemo(() => projectOpsGraphEffects(
    network.selectedStep ? networkMachineRuntimeOperations.readSelectedSnapshot()?.runtimeInput : null,
  ), [network.selectedStep, network.selectedStepIndex]);
  const options = useMemo(() => ({ barsMode, selectedTokenId, view, cue }), [barsMode, selectedTokenId, view, cue]);
  const latest = useRef({ graph, options, effectSignals, selectedStepIndex: network.selectedStepIndex, openEntity, openJurisdiction });
  latest.current = { graph, options, effectSignals, selectedStepIndex: network.selectedStepIndex, openEntity, openJurisdiction };
  useEffect(() => {
    let disposed = false;
    void getXLN().then(runtime => {
      if (disposed || !container.current) return;
      return createOpsGraphScene(container.current, runtime, setSelection, setHover, entityId => {
      ...
      }, cause => setIssue(cause instanceof Error ? cause.message : String(cause)), name => latest.current.openJurisdiction?.(name), setXr);
    }).then(next => {
      if (!next) return;
      if (disposed) { next.dispose(); return; }
      scene.current = next;
      setIssue('');
      try { next.update(latest.current.graph, latest.current.options, latest.current.effectSignals, latest.current.selectedStepIndex); }
  ...
  }, [view.rendererMode, view.antiAlias]);
  useEffect(() => {
    if (!scene.current) return;
    try { scene.current.update(graph, options, effectSignals, network.selectedStepIndex); }
    catch (cause) { setIssue(cause instanceof Error ? cause.message : String(cause)); }
  }, [graph, options, effectSignals, network.selectedStepIndex]);
  ...
  return <section className="ops-graph-panel" data-testid="workspace-graph" data-renderer-request={view.rendererMode} data-selected-step-index={network.selectedStepIndex} data-node-count={graph.nodes.length} data-account-count={graph.accounts.length} data-jurisdiction-count={graph.jMachines.length}>
    <header><strong>Graph3D</strong><span>{graph.sources.length} sources · {graph.nodes.length} {t('network.entities')} · {graph.accounts.length} {t('network.accounts')} · {graph.jMachines.length} {t('workspace.jurisdiction')}</span>
      ...
      <button aria-pressed={xr.active} disabled={!xr.supported} onClick={() => {
        const request = xr.active ? scene.current?.exitXr() : scene.current?.enterXr();
        void request?.catch(cause => setIssue(cause instanceof Error ? cause.message : String(cause)));
      }} title={xr.supported ? '' : t('view.vr.notSupported')} type="button">{t(xr.active ? 'view.vr.exit' : 'view.vr.enter')}</button>
      ...
      <select aria-label={`Graph ${t('view.labels.entity')}`} value={selected ? selectedId : ''} onChange={event => setSelection(event.currentTarget.value ? { kind: 'entity', id: event.currentTarget.value } : null)}><option value="">Select {t('view.labels.entity')}</option>{graph.nodes.map(node => <option key={node.entityId} value={node.entityId}>{node.selected.label}</option>)}</select>
      <select aria-label={`Graph ${t('view.labels.account')}`} value={account ? account.accountId : ''} onChange={event => setSelection(event.currentTarget.value ? { kind: 'account', id: event.currentTarget.value } : null)}><option value="">Select {t('view.labels.account')}</option>{graph.accounts.map(entry => <option key={entry.accountId} value={entry.accountId}>{graph.nodes.find(node => node.entityId === entry.selected.leftEntityId)?.selected.label} ↔ {graph.nodes.find(node => node.entityId === entry.selected.rightEntityId)?.selected.label}</option>)}</select>
      <select aria-label={`Graph ${t('workspace.jurisdiction')}`} value="" disabled={!openJurisdiction} onChange={event => { if (event.currentTarget.value) openJurisdiction?.(event.currentTarget.value); }}><option value="">Open {t('workspace.jurisdiction')}</option>{graph.jMachines.map(machine => <option key={machine.jMachineId} value={machine.selected.name}>{machine.selected.name}</option>)}</select>
    </header>
