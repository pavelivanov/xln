...
export function OpsRuntimeDiagnosticsPanel() {
  ...
  const activeIncidents = filterActiveRuntimeDiagnosticsIncidents(incidents);
  ...
  }, [client, context.historical]);

  const verify = async (): Promise<void> => {
    if (!client || verifying || context.historical) return;
    ...
    try {
      ...
      if (!controller.signal.aborted) setVerification(safeStringify(result, 2));
    } catch (cause) {
  ...
  return (
    <section className="workspace-read-panel workspace-diagnostics" data-testid="runtime-diagnostics-panel">
      ...
      {context.historical ? <p className="workspace-read-state" data-testid="runtime-diagnostics-history">Storage verification and active security incidents require Live. They are not evidence from this recorded frame.</p> : <WorkspaceReadBoundary connected={connected} connection={connection} error={snapshot.error || context.error} loading={snapshot.loading && data === null}>
        ...
        {data ? <div className="workspace-diagnostics-content">
          <dl className="workspace-diagnostics-metrics">
            <div><dt>Adapter</dt><dd data-testid="runtime-diagnostics-adapter">{getRuntimeDiagnosticsAdapterLabel(context.adapter?.mode)}</dd></div>
          ...
          <section aria-label="Security status" data-testid="runtime-security-status">
            ...
            {context.securityIncidents === null ? <p>{context.adapter?.mode === 'embedded' ? 'Local incident observations are loading.' : 'Incident details are unavailable in this remote session.'}</p> : <>
              <p data-testid="runtime-security-summary">{activeIncidents.length ? `${activeIncidents.length} active` : 'No active security incidents.'}</p>
              <div className="workspace-security-incidents">{visibleRuntimeDiagnosticsIncidents(incidents).map(incident => <article className={incident.status === 'active' ? 'is-active' : ''} key={incident.id} data-testid="runtime-security-incident">
                <header><strong>{incident.code}</strong><span>{incident.status}</span></header><p>{incident.summary}</p>
