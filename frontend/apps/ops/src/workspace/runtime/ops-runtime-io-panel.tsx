...
export function OpsRuntimeIoPanel() {
  ...
  return <section className="ops-evidence-panel" data-testid="workspace-runtime-io">
    <header><h2>{t('workspace.runtimeIo')}</h2>{frame ? <span>{t('time.frame')} {frame.state.height} · {frame.state.eReplicas.size} E-replicas · {frame.state.jReplicas.size} J-machines</span> : null}</header>
    {context.error || context.restriction ? <p role={context.error ? 'alert' : 'status'}>{context.error || context.restriction}</p> : frame ? <>
      ...
      <dl className="ops-audit-metrics"><div><dt>{t('network.reserves')} (raw)</dt><dd>{sumRuntimeIoReserves(frame).toString()}</dd></div><div><dt>Collateral (raw)</dt><dd>{sumRuntimeIoCollateral(frame).toString()}</dd></div></dl>
      ...
      {[...frame.state.eReplicas].map(([id, replica]) => <details className="ops-io-detail" key={id}><summary>{replica.state.profile?.name || replica.entityId} · h{replica.state.height} · {replica.state.accounts.size} {t('network.accounts')}</summary><dl><dt>{t('view.labels.entity')} ID</dt><dd><code>{replica.entityId}</code></dd><dt>Signer</dt><dd><code>{replica.signerId}</code></dd><dt>J-Block</dt><dd>{replica.state.lastFinalizedJHeight}</dd></dl><h4>{t('network.reserves')}</h4>{mapToArray(replica.state.reserves).map(([token, amount]) => <p key={token}>Token {token}: <code>{formatBigInt(amount)}</code></p>)}<h4>{t('network.accounts')} (Bilateral)</h4>{mapToArray(replica.state.accounts).map(([peer, account]) => <p key={peer}><code>{peer}</code> · {account.state.deltas.size} deltas</p>)}<p>Mempool: {replica.mempool.length} txs</p></details>)}
    </> : null}
  </section>;
}
