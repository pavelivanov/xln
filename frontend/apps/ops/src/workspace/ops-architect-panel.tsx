...
import { SCENARIO_OPTIONS } from '../../../../packages/runtime-client/src/scenario-player-model';
import { getArchitectScenarioScrollTop } from '../../../../packages/runtime-client/src/architect-panel-view';
import { useWorkspaceEnvironment } from './use-workspace-environment';
import { loadWorkspaceScenario, returnWorkspaceLive, workspaceNetwork } from './ops-workspace-playback';
  ...
  return <section className="ops-evidence-panel ops-architect-panel" data-testid="workspace-architect">
    ...
    {tab === 'solvency' ? <OpsSolvencyPanel /> : <>
      ...
      <form className="ops-panel-controls" onSubmit={event => { event.preventDefault(); void run(); }}>
        <label>Scenario<select aria-label="Architect scenario" disabled={busy || network.loading} value={key} onChange={event => setKey(event.currentTarget.value)}>{keys.map(value => <option key={value} value={value}>{SCENARIO_OPTIONS.find(option => option.runner === value)?.title ?? value}</option>)}</select></label>
        <button disabled={!keys.length || busy || network.loading} type="submit">{busy ? 'Running scenario…' : 'Run scenario'}</button>
