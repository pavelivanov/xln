...
export function OpsGuideChat({ context, messages, setMessages, onClose }: Props) {
  ...
  const connect = async (): Promise<void> => {
    ...
    catalogController.current = request;
    ...
    } finally { if (!request.signal.aborted) { catalogController.current = null; setChecking(false); } }
  };
  useEffect(() => {
    void connect(); textarea.current?.focus();
    return () => { catalogController.current?.abort(); replyController.current?.abort(); };
  }, []);
  useEffect(() => {
    replyController.current?.abort(); replyController.current = null;
    setSending(false); setIssue(''); setMessages([]);
  }, [context.key]);
  ...
  const submit = async (question = input): Promise<void> => {
    ...
    replyController.current = request;
    ...
    try {
      const requestMessages = await buildXlnGuideMessages({ query: question, pathname: window.location.pathname, history, screenContext: context.description, signal: request.signal });
      await streamXlnAssistantReply({ model, messages: requestMessages, signal: request.signal, onContent: content => {
        if (request.signal.aborted) return;
    ...
    } finally { if (replyController.current === request) { replyController.current = null; setSending(false); textarea.current?.focus(); } }
  };
  const stop = (): void => {
    replyController.current?.abort(); replyController.current = null;
    setSending(false); setMessages(current => current.filter((message, index) => index !== current.length - 1 || message.content.trim()));
    textarea.current?.focus();
  };
  return <section className="assistant-panel" data-guide-context={context.key} data-testid="xln-mascot-chat" aria-label="xln assistant" onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); onClose(); } }}>
    <header><div className="identity"><span className="mini-mark" aria-hidden="true" /><span><strong>Ask xln</strong><small>{checking ? 'Connecting…' : catalog ? 'Local AI · public docs' : 'Local AI offline'} · {context.label}</small></span></div><div className="header-actions">{sending ? <button aria-label="Stop answer" onClick={stop} type="button">■</button> : null}<button aria-label="Retry local AI" disabled={checking || sending} onClick={() => { void connect(); }} type="button">↻</button><button aria-label="Close xln assistant" onClick={onClose} type="button">×</button></div></header>
