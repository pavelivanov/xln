import type { DocSection, DocsManifest, ReadingPath } from '../../../packages/ui/src/content/docs-page-model';
    ...
    <>
      ...
      <aside className={isOpen ? 'docs-sidebar is-open' : 'docs-sidebar'} aria-label="Documentation catalog">
        <div className="docs-sidebar-head">
          ...
          <button className="docs-close" type="button" aria-label="Close docs navigation" onClick={onClose}>
            ×
          </button>
        </div>
        ...
        {manifest ? (
          <div className="docs-counts" aria-label="Document counts">
            <span>
              <strong>{manifest.counts.live}</strong> live
            </span>
            <span>
              <strong>{manifest.counts.archive}</strong> archive
            </span>
            <span>
              <strong>{manifest.counts.total}</strong> total
            </span>
          </div>
        ) : null}
        ...
        <div className="docs-scope" aria-label="Catalog scope">
          <button
            type="button"
            className={!showArchive ? 'is-active' : undefined}
          ...
          <div className="docs-sidebar-content">
            {!isFiltering ? (
              <section className="docs-nav-section docs-reading-paths">
                <div className="docs-section-label">
                  ...
                  <span>{manifest.readingPaths.length.toString().padStart(2, '0')}</span>
                </div>
                <div className="docs-path-list">
                  {manifest.readingPaths.map(path => (
                    <ReadingPathButton path={path} onOpenDoc={onOpenDoc} key={path.id} />
                  ))}
                </div>
              </section>
            ) : null}

            {!isFiltering && manifest.featured.length > 0 ? (
              <section className="docs-nav-section docs-featured">
                <div className="docs-section-label">
                  <span>Featured</span>
                  <span>{manifest.featured.length.toString().padStart(2, '0')}</span>
                </div>
                <div className="docs-link-list">
            ...
            <nav aria-label="Documents">
              ...
              {visibleSections.length === 0 ? (
                <p className="docs-no-results" data-testid="docs-no-results">
                  No documents match this catalog view.
                </p>
              ) : null}
            </nav>
          </div>
