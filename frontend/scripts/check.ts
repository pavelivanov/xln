...
export const createCheckCommands = (request: CheckRequest): readonly CommandSpec[] => {
  ...
  if (request.level === 'frontend')
    commands.push({
      label: request.surfaceIds.length === 4 ? 'frontend-units' : 'frontend-contracts',
      cwd: fileURLToPath(new URL('../..', import.meta.url)),
      argv:
        request.surfaceIds.length === 4
  ...
  for (const surface of request.surfaceIds)
    commands.push({ label: `prepare-${surface}`, argv: ['bun', 'scripts/prepare.ts', `--surface=${surface}`] });
  commands.push(...createBuildCommands(request.surfaceIds));
  if (request.level === 'slice' && request.spec)
    commands.push({
      label: 'slice-browser',
      argv: ['bunx', 'playwright', 'test', '--config', 'playwright.react.config.ts', request.spec],
      environment: { PLAYWRIGHT_REACT_SURFACE: request.surfaceIds[0] ?? '' },
    });
  if (request.level === 'frontend' && request.surfaceIds.length === 4)
    commands.push({ label: 'assemble-candidate', argv: ['bun', 'scripts/assemble.ts'] });
  return commands;
};
...
const run = async (): Promise<void> => {
  ...
  if (request.changedFrom) {
    const sha = (
      await gitOutput(['rev-parse', '--verify', '--end-of-options', `${request.changedFrom}^{commit}`])
    ).trim();
    ...
    request = {
      ...request,
      surfaceIds: selectChangedSurfaces(changed.flatMap(output => output.split('\0').filter(Boolean))),
    };
  }
  ...
  if (request.explain) {
    console.info(
      commands
        .map(({ label, argv, environment }) =>
          `${label}: ${Object.entries(environment ?? {})
