import type { SurfaceId } from '../../packages/frontend-release/surfaces';

export type PlatformInventoryOwner = SurfaceId | 'assembly';
export type PlatformInventoryStatus = 'implemented' | 'owned-for-later-wp';
export type PlatformInterface =
  | 'asset'
  | 'broadcast-channel'
  | 'cache-storage'
  | 'capacitor'
  | 'dedicated-worker'
  | 'desktop-shell'
  | 'extension-shell'
  | 'indexed-db'
  | 'local-storage'
  | 'registry'
  | 'release-artifact'
  | 'route'
  | 'service-worker'
  | 'session-storage'
  | 'verification-command'
  | 'web-locks';

export type PlatformInventoryEntry = Readonly<{
  id: string;
  owner: PlatformInventoryOwner;
  workPackage: `WP${number}`;
  status: PlatformInventoryStatus;
  interfaces: readonly PlatformInterface[];
  sources: readonly string[];
  consumers: readonly string[];
  evidence: readonly string[];
}>;

export const PLATFORM_INVENTORY = [
  {
    id: 'route-capability-ownership',
    owner: 'assembly',
    workPackage: 'WP0',
    status: 'implemented',
    interfaces: ['route', 'registry'],
    sources: [
      'packages/frontend-release/surfaces.ts',
      'frontend/config/capabilities.ts',
      'frontend/config/development-gateway.ts',
    ],
    consumers: [
      'frontend/scripts/dev/dev-gateway.ts',
      'frontend/scripts/release/candidate-release.ts',
    ],
    evidence: [
      'tests/frontend/tooling/frontend-route-ownership.test.ts',
      'tests/frontend/tooling/audit/frontend-capability-coverage.test.ts',
      'tests/frontend/tooling/frontend-development-gateway.test.ts',
    ],
  },
  {
    id: 'frontend-verification-commands',
    owner: 'assembly',
    workPackage: 'WP0',
    status: 'implemented',
    interfaces: ['verification-command'],
    sources: ['package.json', 'frontend/package.json', 'frontend/scripts/check.ts'],
    consumers: ['frontend/scripts/build.ts', 'frontend/scripts/assemble.ts'],
    evidence: [
      'tests/frontend/tooling/build/frontend-check-scope.test.ts',
      'tests/frontend/tooling/build/frontend-build-isolation.test.ts',
    ],
  },
  {
    id: 'deterministic-generated-inputs',
    owner: 'assembly',
    workPackage: 'WP2',
    status: 'implemented',
    interfaces: ['asset', 'release-artifact', 'registry'],
    sources: [
      'frontend/config/generated-inputs.ts',
      'frontend/scripts/inputs/generated-inputs.ts',
      'frontend/scripts/inputs/generated-input-manifest.ts',
    ],
    consumers: ['frontend/scripts/prepare.ts', 'frontend/scripts/release/candidate-release.ts'],
    evidence: [
      'tests/frontend/tooling/frontend-input-ownership.test.ts',
      'tests/frontend/tooling/build/frontend-generated-inputs.test.ts',
      'tests/frontend/tooling/build/frontend-candidate-assembly.test.ts',
    ],
  },
  {
    id: 'runtime-adapter-session-storage',
    owner: 'wallet',
    workPackage: 'WP5',
    status: 'implemented',
    interfaces: ['local-storage', 'session-storage'],
    sources: [
      'frontend/packages/browser/src/runtime/session/runtime-adapter-session.ts',
      'frontend/packages/browser/src/runtime/session/remote-runtime-import.ts',
    ],
    consumers: [
      'frontend/apps/wallet/src/app-shell.tsx',
      'frontend/apps/wallet/src/address/wallet-address.tsx',
    ],
    evidence: [
      'tests/frontend/runtime/session/runtime-adapter-session.test.ts',
      'tests/frontend/onboarding/runtime/remote-runtime-import.test.ts',
      'tests/frontend/runtime/wallet/frontend-wallet-address.test.ts',
      'frontend/tests/react-candidate/wallet.spec.ts',
    ],
  },
  {
    id: 'wallet-durable-custody-storage',
    owner: 'wallet',
    workPackage: 'WP9',
    status: 'owned-for-later-wp',
    interfaces: ['indexed-db', 'local-storage', 'web-locks'],
    sources: [
      'frontend/packages/browser/src/vault/vault-protection.ts',
      'frontend/packages/browser/src/commands/runtime-command-intent.ts',
      'frontend/packages/browser/src/commands/runtime-command-intent-codec.ts',
      'frontend/packages/browser/src/commands/runtime-command-journal-indexed-db.ts',
      'frontend/packages/browser/src/commands/runtime-command-journal-keyring.ts',
      'frontend/packages/browser/src/commands/runtime-command-journal-storage.ts',
      'frontend/bridges/vault/vault-store.ts',
    ],
    consumers: ['frontend/bridges/wallet/canonical/wallet-canonical-vault-runtime.ts', 'frontend/apps/wallet/src/runtime/wallet-embedded-runtime.ts'],
    evidence: [
      'tests/frontend/security/vault-protection.test.ts',
      'tests/frontend/recovery/storage-schema-recovery.test.ts',
    ],
  },
  {
    id: 'runtime-tab-coordination',
    owner: 'wallet',
    workPackage: 'WP5',
    status: 'implemented',
    interfaces: ['broadcast-channel', 'session-storage', 'web-locks'],
    sources: [
      'frontend/packages/browser/src/active-tab-lock.ts',
      'frontend/packages/browser/src/active-tab-lock-support.ts',
    ],
    consumers: [
      'frontend/packages/browser/src/active-tab-lock-store.ts',
      'frontend/apps/wallet/src/runtime/wallet-embedded-runtime.ts',
      'frontend/apps/wallet/src/onboarding/wallet-identity-opening.ts',
    ],
    evidence: [
      'tests/frontend/runtime/session/active-tab-lock.test.ts',
      'tests/e2e/runtime/e2e-active-tab-lock.spec.ts',
    ],
  },
  {
    id: 'embedded-runtime-browser-session',
    owner: 'wallet',
    workPackage: 'WP5',
    status: 'implemented',
    interfaces: ['asset', 'indexed-db', 'web-locks', 'worker', 'web-crypto'],
    sources: [
      'frontend/packages/browser/src/runtime/session/runtime-module-loader.ts',
      'frontend/packages/browser/src/runtime/wallet-embedded-runtime-session.ts',
      'frontend/packages/browser/src/runtime/wallet-runtime-suspension.ts',
      'frontend/packages/browser/src/wallet/wallet-vault-storage.ts',
      'frontend/packages/browser/src/recovery/wallet-recovery-selection-session.ts',
      'frontend/packages/browser/src/recovery/wallet-recovery-services.ts',
      'frontend/packages/browser/src/recovery/recovery-coverage-view.ts',
      'frontend/packages/browser/src/identity/wallet-brainvault-material-session.ts',
      'frontend/packages/browser/src/identity/wallet-brainvault-opening.ts',
      'frontend/bridges/runtime/browser/browser-runtime-adapter.ts',
      'frontend/bridges/runtime/browser/browser-runtime-bootstrap.ts',
      'frontend/apps/wallet/src/runtime/wallet-embedded-runtime.ts',
      'frontend/bridges/wallet/brainvault/wallet-brainvault-browser-derivation.ts',
      'frontend/bridges/wallet/brainvault/wallet-brainvault-material-finalization.ts',
      'frontend/bridges/wallet/brainvault/wallet-brainvault-worker-runtime.ts',
      'frontend/bridges/wallet/canonical/wallet-canonical-vault-runtime.ts',
      'frontend/bridges/wallet/canonical/wallet-canonical-recovery-services.ts',
      'frontend/apps/wallet/src/recovery/wallet-recovery-services-source.ts',
    ],
    consumers: [
      'frontend/apps/wallet/src/app-shell.tsx',
      'frontend/apps/wallet/src/runtime/wallet-runtime-read-boundary.ts',
      'frontend/bridges/runtime/xln-runtime-loader.ts',
      'frontend/bridges/vault/vault-store.ts',
    ],
    evidence: [
      'tests/frontend/runtime/session/runtime-module-loader.test.ts',
      'tests/frontend/runtime/wallet/lifecycle/wallet-embedded-runtime-session.test.ts',
      'tests/frontend/onboarding/recovery/wallet-recovery-selection-session.test.ts',
      'tests/frontend/onboarding/brainvault/wallet-brainvault-material-session.test.ts',
      'tests/frontend/onboarding/brainvault/frontend-wallet-brainvault-derivation.test.ts',
      'tests/frontend/onboarding/recovery/frontend-wallet-recovery-file.test.ts',
      'tests/frontend/onboarding/recovery/frontend-wallet-recovery-services.test.ts',
      'tests/frontend/runtime/wallet/wallet-runtime-suspension.test.ts',
      'tests/frontend/runtime/wallet/frontend-wallet-app-shell.test.ts',
    ],
  },
  {
    id: 'browser-runtime-reset',
    owner: 'wallet',
    workPackage: 'WP5',
    status: 'implemented',
    interfaces: ['cache-storage', 'indexed-db', 'service-worker'],
    sources: [
      'frontend/packages/browser/src/runtime/session/browser-runtime-reset.ts',
      'frontend/packages/browser/src/hard-reset-request.ts',
    ],
    consumers: [
      'frontend/apps/wallet/src/testnet/testnet-page.tsx',
    ],
    evidence: [
      'tests/frontend/control/reset-everything.test.ts',
      'tests/frontend/tooling/pilots/frontend-testnet-pilot.test.ts',
    ],
  },
  {
    id: 'brainvault-worker-boundary',
    owner: 'wallet',
    workPackage: 'WP5',
    status: 'implemented',
    interfaces: ['dedicated-worker', 'local-storage'],
    sources: [
      'frontend/packages/browser/src/identity/wallet-brainvault-worker-validation.ts',
      'frontend/packages/browser/src/identity/wallet-brainvault-worker-scheduling.ts',
      'frontend/packages/browser/src/identity/wallet-brainvault-worker-resilience.ts',
    ],
    consumers: [
      'frontend/apps/wallet/src/settings/wallet-settings-model.ts',
    ],
    evidence: [
      'tests/frontend/onboarding/brainvault/wallet-brainvault-worker-validation.test.ts',
      'tests/frontend/onboarding/brainvault/wallet-brainvault-worker-scheduling.test.ts',
      'tests/frontend/onboarding/brainvault/wallet-brainvault-worker-resilience.test.ts',
    ],
  },
  {
    id: 'push-wake-service-worker',
    owner: 'wallet',
    workPackage: 'WP8',
    status: 'implemented',
    interfaces: ['asset', 'local-storage', 'service-worker'],
    sources: [
      'frontend/static/push-wake-sw.js',
      'frontend/bridges/vault/push-wake-registration.ts',
      'frontend/packages/browser/src/wallet/wallet-push-wake.ts',
      'frontend/bridges/wallet/canonical/wallet-canonical-push-wake.ts',
    ],
    consumers: [
      'frontend/apps/wallet/src/push-wake/wallet-push-wake.tsx',
    ],
    evidence: [
      'tests/frontend/recovery/push-wake-registration.test.ts',
      'tests/frontend/onboarding/runtime/frontend-wallet-push-wake.test.ts',
      'frontend/tests/react-candidate/wallet.spec.ts',
    ],
  },
  {
    id: 'native-and-packaged-consumers',
    owner: 'wallet',
    workPackage: 'WP8',
    status: 'implemented',
    interfaces: ['capacitor', 'desktop-shell', 'extension-shell', 'release-artifact'],
    sources: [
      'frontend/capacitor.config.ts',
      'frontend/packages/browser/src/native/capacitor.ts',
      'frontend/packages/browser/src/native/deeplink.ts',
      'native/desktop/main.cjs',
      'native/extension/manifest.json',
      'scripts/native/wallet-candidate-manifest.ts',
      'scripts/native/capacitor-shell-candidate-manifest.ts',
      'scripts/native/packaged-shell-candidate-manifest.ts',
      'scripts/native/policy/packaged-shell-policy.ts',
      'scripts/native/policy/regular-tree.ts',
    ],
    consumers: [
      'scripts/native/stage-wallet-candidate.ts',
      'scripts/native/smoke-capacitor-candidate.ts',
      'scripts/native/copy-capacitor-shell-candidate.ts',
      'scripts/native/copy-packaged-shell-candidate.ts',
      'frontend/config/playwright/playwright.packaged.config.ts',
    ],
    evidence: [
      'native/__tests__/capacitor/capacitor-config.test.ts',
      'native/__tests__/native-deeplink.test.ts',
      'native/__tests__/desktop-security.test.ts',
      'native/__tests__/extension-security.test.ts',
      'native/__tests__/wallet-candidate-staging.test.ts',
      'native/__tests__/capacitor/capacitor-shell-candidate.test.ts',
      'native/__tests__/packaged-shell-candidate.test.ts',
      'frontend/tests/packaged-candidate/extension.spec.ts',
    ],
  },
  {
    id: 'ops-health-browser-boundary',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route'],
    sources: [
      'frontend/packages/runtime-client/src/admin-health.ts',
      'frontend/packages/runtime-client/src/runtime/rpc-health.ts',
      'frontend/packages/runtime-client/src/boundary.ts',
      'frontend/apps/ops/src/health/ops-health-source.ts',
      'frontend/apps/ops/src/health/ops-health-runtime.ts',
    ],
    consumers: ['frontend/apps/ops/src/health/ops-health.tsx'],
    evidence: ['tests/frontend/ops/health/frontend-ops-health.test.ts'],
  },
  {
    id: 'ops-hlt-browser-boundary',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route'],
    sources: [
      'frontend/packages/runtime-client/src/qa/qa-hlt.ts',
      'frontend/apps/ops/src/hlt/ops-hlt-model.ts',
      'frontend/apps/ops/src/hlt/ops-hlt-source.ts',
      'frontend/apps/ops/src/hlt/ops-hlt-runtime.ts',
    ],
    consumers: [
      'frontend/apps/ops/src/hlt/ops-hlt-controls.tsx',
      'frontend/apps/ops/src/hlt/ops-hlt-progress.tsx',
      'frontend/apps/ops/src/hlt/ops-hlt.tsx',
    ],
    evidence: ['tests/frontend/ops/frontend-ops-hlt.test.ts'],
  },
  {
    id: 'ops-qa-browser-boundary',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route', 'session-storage'],
    sources: [
      'core/config/qa.ts',
      'frontend/packages/browser/src/qa-api-client.ts',
      'frontend/packages/runtime-client/src/qa/evidence/qa-admin-evidence.ts',
      'frontend/packages/runtime-client/src/qa/evidence/qa-boundary.ts',
      'frontend/packages/runtime-client/src/qa/evidence/qa-cockpit-helpers.ts',
      'frontend/packages/runtime-client/src/qa/evidence/qa-performance-trend.ts',
      'frontend/packages/runtime-client/src/qa/qa-scenario-player.ts',
      'frontend/packages/runtime-client/src/qa/evidence/qa-test-ledger.ts',
      'frontend/packages/runtime-client/src/qa/evidence/qa-types.ts',
      'frontend/apps/ops/src/qa/ops-qa-model.ts',
      'frontend/apps/ops/src/qa/ops-qa-source.ts',
      'frontend/apps/ops/src/qa/ops-qa-actions.ts',
      'frontend/apps/ops/src/qa/ops-qa-runtime.ts',
    ],
    consumers: [
      'frontend/apps/ops/src/qa/ops-qa.tsx',
      'frontend/apps/ops/src/qa/evidence/ops-qa-run.tsx',
      'frontend/apps/ops/src/qa/media/ops-qa-gallery.tsx',
      'frontend/apps/ops/src/qa/evidence/ops-qa-history.tsx',
    ],
    evidence: [
      'tests/frontend/ops/frontend-ops-qa.test.ts',
      'tests/frontend/qa/boundary.test.ts',
      'tests/frontend/qa/qa-test-ledger.test.ts',
    ],
  },
  {
    id: 'ops-runs-scenarios-browser-boundary',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route'],
    sources: [
      'frontend/packages/browser/src/runtime/session/runtime-scenario-source.ts',
      'frontend/packages/runtime-client/src/scenario/scenario-player-model.ts',
      'frontend/packages/runtime-client/src/scenario/scenario-runtime.ts',
      'frontend/apps/ops/src/runs/ops-runs-model.ts',
      'frontend/apps/ops/src/runs/ops-runs-source.ts',
      'frontend/apps/ops/src/runs/ops-runs-runtime.ts',
      'frontend/apps/ops/src/scenarios/ops-scenarios-runtime.ts',
      'frontend/apps/wallet/src/scenario-preview/wallet-scenario-preview-runtime.ts',
    ],
    consumers: [
      'frontend/apps/ops/src/runs/ops-runs.tsx',
      'frontend/apps/ops/src/scenarios/ops-scenarios.tsx',
      'frontend/apps/wallet/src/scenario-preview/wallet-scenario-preview.tsx',
    ],
    evidence: [
      'core/__tests__/scenarios/browser-api.test.ts',
      'tests/frontend/ops/frontend-ops-runs.test.ts',
      'tests/frontend/ops/frontend-ops-scenarios.test.ts',
      'tests/frontend/runtime/runtime-scenario-source.test.ts',
    ],
  },
  {
    id: 'ops-quorum-dashboard',
    owner: 'ops',
    workPackage: 'WP9',
    status: 'implemented',
    interfaces: ['registry', 'route'],
    sources: [
      'audits/registry.json',
      'frontend/packages/runtime-client/src/qa/quorum/qa-quorum-history.ts',
      'frontend/packages/runtime-client/src/qa/quorum/qa-quorum-model.ts',
      'frontend/packages/runtime-client/src/qa/quorum/qa-quorum-registry.ts',
      'frontend/packages/runtime-client/src/qa/quorum/qa-quorum-types.ts',
      'frontend/apps/ops/src/quorum/ops-quorum-source.ts',
    ],
    consumers: [
      'frontend/apps/ops/src/quorum/ops-quorum.tsx',
    ],
    evidence: [
      'tests/frontend/ops/frontend-ops-quorum.test.ts',
      'frontend/tests/react-candidate/ops/ops.spec.ts',
    ],
  },
  {
    id: 'ops-ai-browser-boundary',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route'],
    sources: [
      'frontend/apps/ops/src/ai/ops-ai-decode.ts',
      'frontend/apps/ops/src/ai/ops-ai-model.ts',
      'frontend/apps/ops/src/ai/ops-ai-source.ts',
      'frontend/apps/ops/src/ai/ops-ai-runtime.ts',
      'frontend/packages/ui/src/stable-compare.ts',
    ],
    consumers: [
      'frontend/apps/ops/src/ai/ops-ai.tsx',
      'frontend/apps/ops/src/ai/ops-ai-header.tsx',
      'frontend/apps/ops/src/ai/ops-ai-messages.tsx',
      'frontend/apps/ops/src/ai/ops-ai-sidebar.tsx',
    ],
    evidence: [
      'tests/frontend/ops/frontend-ops-ai.test.ts',
    ],
  },
  {
    id: 'ops-workspace-boot-boundary',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route'],
    sources: [
      'frontend/packages/runtime-client/src/scenario/embed-boot-model.ts',
      'frontend/packages/runtime-client/src/scenario/demo-playback-intent.ts',
    ],
    consumers: [
      'frontend/packages/browser/src/graph/network-machine-demo-store.ts',
    ],
    evidence: [
      'tests/frontend/runtime/embed-boot-model.test.ts',
      'tests/frontend/graph/timeline/network-machine-demo.test.ts',
    ],
  },
  {
    id: 'ops-workspace-panel-projections',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['route'],
    sources: [
      'frontend/packages/runtime-client/src/panels/console-panel-view.ts',
      'frontend/packages/runtime-client/src/runtime/view/runtime-io-panel-view.ts',
      'frontend/packages/runtime-client/src/panels/solvency-panel-view.ts',
      'frontend/packages/runtime-client/src/runtime/view/runtime-diagnostics-panel-view.ts',
      'frontend/packages/runtime-client/src/panels/gossip-panel-view.ts',
      'frontend/packages/runtime-client/src/scenario/time-machine-transport.ts',
      'frontend/packages/runtime-client/src/panels/jurisdiction-panel-view.ts',
      'frontend/packages/runtime-client/src/panels/settings-panel-view.ts',
      'frontend/packages/runtime-client/src/panels/architect-panel-view.ts',
      'frontend/packages/runtime-client/src/graph/graph3d-entity-panel-view.ts',
      'frontend/packages/runtime-client/src/graph/graph3d-viewport-view.ts',
      'frontend/packages/runtime-client/src/graph/graph3d-scene-input.ts',
    ],
    consumers: [
      'frontend/bridges/runtime/runtime-history-store.ts',
      'frontend/packages/ui/src/graph/runtime-graph-projection.ts',
      'frontend/packages/browser/src/graph/runtime-graph-control-store.ts',
    ],
    evidence: [
      'tests/frontend/runtime/panels/console-panel-view.test.ts',
      'tests/frontend/runtime/panels/runtime-io-panel-view.test.ts',
      'tests/frontend/runtime/panels/solvency-panel-view.test.ts',
      'tests/frontend/runtime/panels/runtime-diagnostics-panel-view.test.ts',
      'tests/frontend/runtime/panels/gossip-panel-view.test.ts',
      'tests/frontend/runtime/timeline/time-machine-transport.test.ts',
      'tests/frontend/runtime/panels/jurisdiction-panel-view.test.ts',
      'tests/frontend/runtime/panels/settings-panel-view.test.ts',
      'tests/frontend/runtime/panels/architect-panel-view.test.ts',
      'tests/frontend/graph/graph3d-entity-panel-view.test.ts',
      'tests/frontend/graph/graph3d-viewport-view.test.ts',
      'tests/frontend/graph/graph3d-scene-input.test.ts',
    ],
  },
  {
    id: 'ops-entity-workspace-navigation',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['local-storage', 'route', 'registry', 'session-storage', 'ui'],
    sources: [
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-navigation.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-context.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-activity.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-consensus-evidence.ts',
      'frontend/packages/runtime-client/src/entity/profile/entity-workspace-hub-policy.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-accounts.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-ownership.ts',
      'frontend/packages/runtime-client/src/entity/profile/entity-workspace-profile.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-reserves.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-settings-summary.ts',
      'frontend/packages/runtime-client/src/entity/workspace/entity-workspace-time-machine.ts',
      'frontend/packages/browser/src/preferences/display-preferences.ts',
      'frontend/packages/browser/src/preferences/theme-document.ts',
      'frontend/packages/ui/src/theme-model.ts',
      'frontend/packages/ui/src/entity/settings/entity-workspace-display.ts',
      'frontend/packages/ui/src/entity/settings/entity-workspace-display-panel.tsx',
      'frontend/packages/ui/src/entity/settings/entity-workspace-display-panel.css',
      'frontend/packages/ui/src/entity/settings/entity-workspace-settings-stage.tsx',
      'frontend/packages/ui/src/entity/settings/entity-workspace-settings-stage.css',
      'frontend/packages/ui/src/entity/entity-workspace-time-machine.tsx',
      'frontend/packages/ui/src/entity/entity-workspace-time-machine.css',
      'frontend/packages/ui/src/entity/activity/entity-workspace-activity-panel.tsx',
      'frontend/packages/ui/src/entity/activity/entity-workspace-activity-row.tsx',
      'frontend/packages/ui/src/entity/activity/entity-workspace-activity-tone.ts',
      'frontend/packages/ui/src/entity/activity/entity-workspace-activity-panel.css',
      'frontend/packages/ui/src/entity/accounts/entity-workspace-consensus-panel.tsx',
      'frontend/packages/ui/src/entity/accounts/entity-workspace-consensus-panel.css',
      'frontend/packages/ui/src/entity/accounts/entity-workspace-accounts-panel.tsx',
      'frontend/packages/ui/src/entity/accounts/entity-workspace-accounts-panel.css',
      'frontend/packages/ui/src/entity/profile/entity-workspace-profile-panel.tsx',
      'frontend/packages/ui/src/entity/profile/entity-workspace-profile-panel.css',
      'frontend/packages/ui/src/entity/accounts/entity-workspace-reserves-panel.tsx',
      'frontend/packages/ui/src/entity/accounts/entity-workspace-reserves-panel.css',
      'frontend/packages/ui/src/entity/entity-workspace-shell.tsx',
      'frontend/packages/ui/src/entity/entity-workspace-shell.css',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace.tsx',
      'frontend/apps/ops/src/ops-display-preferences.ts',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace-activity-controller.ts',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace-history.ts',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace-history-controller.ts',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace-projection.ts',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace-source.ts',
      'frontend/apps/ops/src/entity-workspace/ops-entity-workspace-runtime.ts',
    ],
    consumers: [
      'frontend/apps/ops/src/ops-app.tsx',
      'frontend/apps/ops/src/main.tsx',
      'frontend/apps/ops/src/ops-model.ts',
    ],
    evidence: [
      'tests/frontend/workspace/panels/entity-panel-routing.test.ts',
      'tests/frontend/workspace/panels/entity-panel-display.test.ts',
      'tests/frontend/workspace/entity/entity-workspace-context.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-activity-ledger.test.ts',
      'tests/frontend/workspace/entity/entity-workspace-consensus-evidence.test.ts',
      'tests/frontend/workspace/entity/entity-workspace-accounts.test.ts',
      'tests/frontend/workspace/entity/entity-workspace-ownership.test.ts',
      'tests/frontend/workspace/entity/entity-workspace-profile.test.ts',
      'tests/frontend/workspace/entity/entity-workspace-reserves.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-account-commitments.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-hub-policy.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-settings-summary.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-workspace.test.ts',
      'tests/frontend/ops/frontend-ops-display-preferences.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-time-machine.test.ts',
      'tests/frontend/ops/entity/frontend-ops-entity-runtime-read.test.ts',
      'tests/e2e/runtime/e2e-react-entity-workspace.spec.ts',
      'frontend/tests/react-candidate/ops/ops.spec.ts',
    ],
  },
  {
    id: 'ops-workspace-dock-layout',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['local-storage', 'registry'],
    sources: ['frontend/packages/runtime-client/src/workspace-dock-layout.ts'],
    consumers: [
      'frontend/packages/ui/src/workspace/workspace-dock.tsx',
    ],
    evidence: ['tests/frontend/workspace/workspace-dock-layout.test.ts'],
  },
  {
    id: 'ops-workspace-graph3d-lifecycle',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'implemented',
    interfaces: ['registry'],
    sources: [
      'frontend/packages/ui/src/graph/three/core/graph3d-lifecycle.ts',
      'frontend/packages/ui/src/graph/three/core/graph3d-renderer.ts',
      'frontend/packages/ui/src/graph/three/visuals/graph3d-scene-primitives.ts',
      'frontend/packages/ui/src/graph/three/visuals/graph3d-visual-effects.ts',
      'frontend/packages/ui/src/graph/three/visuals/graph3d-entity-visuals.ts',
      'frontend/packages/ui/src/graph/three/visuals/graph3d-account-visuals.ts',
      'frontend/packages/ui/src/graph/three/core/graph3d-camera.ts',
      'frontend/packages/ui/src/graph/three/interaction/graph3d-hover.ts',
      'frontend/packages/ui/src/graph/three/interaction/graph3d-interaction.ts',
      'frontend/packages/browser/src/runtime/debug-surface.ts',
    ],
    consumers: [
      'frontend/packages/ui/src/graph/three/visuals/graph3d-visuals.ts',
    ],
    evidence: [
      'tests/frontend/graph/rendering/graph3d-lifecycle.test.ts',
      'tests/frontend/graph/rendering/graph3d-renderer.test.ts',
      'tests/frontend/graph/rendering/graph3d-scene-primitives.test.ts',
      'tests/frontend/graph/rendering/graph3d-visual-effects.test.ts',
      'tests/frontend/graph/rendering/graph3d-entity-visuals.test.ts',
      'tests/frontend/graph/rendering/graph3d-account-visuals.test.ts',
      'tests/frontend/graph/rendering/graph3d-camera.test.ts',
      'tests/frontend/graph/rendering/graph3d-hover.test.ts',
      'tests/frontend/graph/rendering/graph3d-interaction.test.ts',
    ],
  },
  {
    id: 'ops-workspace-registries',
    owner: 'ops',
    workPackage: 'WP7',
    status: 'owned-for-later-wp',
    interfaces: ['local-storage', 'registry'],
    sources: [
      'frontend/packages/ui/src/workspace/command-palette-view.ts',
      'frontend/packages/ui/src/workspace/command-palette-suggestions.ts',
      'frontend/apps/ops/src/workspace/ops-command-palette.tsx',
      'frontend/packages/browser/src/localization/index.ts',
      'frontend/bridges/workspace-localization.ts',
      'frontend/bridges/workspace-localization-react.ts',
      'frontend/apps/ops/src/workspace/ops-panel-title.ts',
    ],
    consumers: ['frontend/apps/ops/src/workspace/ops-workspace.tsx', 'frontend/apps/wallet/src/app-shell.tsx'],
    evidence: [
      'tests/frontend/command-palette-view.test.ts',
      'tests/frontend/ops/frontend-workspace-localization.test.ts',
      'frontend/tests/react-candidate/ops/workspace/ops-workspace-localization.spec.ts',
      'frontend/tests/react-candidate/wallet/wallet-localization.spec.ts',
      'frontend/tests/react-candidate/ops/ops-public-embed.spec.ts',
      'tests/frontend/workspace/dockroot-runtime-projection.test.ts',
    ],
  },
  {
    id: 'pwa-candidate-lifecycle',
    owner: 'wallet',
    workPackage: 'WP8',
    status: 'implemented',
    interfaces: ['cache-storage', 'release-artifact', 'service-worker', 'verification-command'],
    sources: [
      'frontend/scripts/pwa/pwa-candidate.ts',
      'frontend/scripts/pwa/pwa-candidate-smoke-server.ts',
      'frontend/playwright.pwa.config.ts',
    ],
    consumers: ['frontend/package.json'],
    evidence: [
      'tests/frontend/tooling/build/frontend-pwa-candidate.test.ts',
      'frontend/tests/pwa-candidate/lifecycle.spec.ts',
    ],
  },
  {
    id: 'candidate-release-consumers',
    owner: 'assembly',
    workPackage: 'WP8',
    status: 'implemented',
    interfaces: ['release-artifact', 'verification-command'],
    sources: [
      'frontend/scripts/release/candidate-release.ts',
      'frontend/scripts/release/candidate-release-verifier.ts',
      'packages/frontend-release/manifest.ts',
      'packages/frontend-release/verify.ts',
      'packages/frontend-release/http.ts',
      'packages/frontend-release/public-routes.ts',
      'packages/frontend-release/serve.ts',
    ],
    consumers: [
      'scripts/native/wallet-candidate-manifest.ts',
      'scripts/native/stage-wallet-candidate.ts',
      'scripts/native/capacitor-candidate.ts',
      'scripts/native/smoke-capacitor-candidate.ts',
      'scripts/native/capacitor-shell-candidate-manifest.ts',
      'scripts/native/copy-capacitor-shell-candidate.ts',
      'scripts/native/packaged-shell-candidate-manifest.ts',
      'scripts/native/copy-packaged-shell-candidate.ts',
      'frontend/config/playwright/playwright.packaged.config.ts',
      'frontend/scripts/pwa/pwa-candidate.ts',
      'packages/frontend-release/deployment.ts',
      'frontend/scripts/deployment/deployment-candidate-cli.ts',
      'frontend/scripts/deployment/deployment-candidate-smoke-server.ts',
      'frontend/scripts/release/artifact-browser-server.ts',
      'frontend/scripts/preview.ts',
      'frontend/scripts/release/verified-release-server.ts',
      'scripts/release/build-xlnfinance-package.ts',
      'core/api/server/index.ts',
      'frontend/scripts/testing/test-artifact-candidate.ts',
      'frontend/playwright.artifact.config.ts',
    ],
    evidence: [
      'tests/frontend/tooling/build/frontend-candidate-assembly.test.ts',
      'tests/frontend/tooling/build/frontend-candidate-release-verifier.test.ts',
      'tests/frontend/tooling/build/frontend-release-preview.test.ts',
      'frontend/scripts/release/check-preview-consumer.ts',
      'native/__tests__/wallet-candidate-staging.test.ts',
      'native/__tests__/capacitor/capacitor-candidate-smoke.test.ts',
      'native/__tests__/capacitor/capacitor-shell-candidate.test.ts',
      'native/__tests__/packaged-shell-candidate.test.ts',
      'frontend/tests/packaged-candidate/extension.spec.ts',
      'tests/frontend/tooling/build/frontend-deployment-candidate.test.ts',
      'frontend/tests/deployment-candidate/lifecycle.spec.ts',
      'frontend/tests/artifact-candidate/artifact-browser.spec.ts',
      'tests/frontend/tooling/content-security-policy.test.ts',
    ],
  },
  {
    id: 'retained-parity-audit',
    owner: 'assembly',
    workPackage: 'WP9',
    status: 'implemented',
    interfaces: ['registry', 'route', 'verification-command'],
    sources: [
      'frontend/config/capabilities.ts',
      'frontend/config/parity-audit.ts',
      'packages/frontend-release/surfaces.ts',
      'frontend/config/wallet-flow-audit.ts',
    ],
    consumers: ['frontend/scripts/checks/parity-audit.ts'],
    evidence: ['tests/frontend/tooling/audit/frontend-parity-audit.test.ts'],
  },
  {
    id: 'canonical-release-consumers',
    owner: 'assembly',
    workPackage: 'WP10',
    status: 'owned-for-later-wp',
    interfaces: ['desktop-shell', 'extension-shell', 'release-artifact'],
    sources: [
      'frontend/scripts/release/candidate-release.ts',
      'frontend/scripts/release/candidate-release-verifier.ts',
    ],
    consumers: [
      'scripts/release/build-xlnfinance-package.ts',
      'scripts/native/build-platforms.ts',
      'scripts/native/native-release-build.ts',
      '.github/workflows/distribution-release.yml',
      'scripts/deployment/deploy-platform.sh',
    ],
    evidence: [
      'native/__tests__/native-build-options.test.ts',
      'tests/frontend/tooling/build/frontend-distribution-consumers.test.ts',
      'tests/frontend/tooling/build/frontend-candidate-release-verifier.test.ts',
    ],
  },
] as const satisfies readonly PlatformInventoryEntry[];
