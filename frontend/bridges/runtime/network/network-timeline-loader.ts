...
import type { Runtime } from '../runtime-store';
import { getRuntimeControllerAdapter } from '../runtime-controller-store';
import {
...
} from '../../../packages/runtime-client/src/graph/runtime-graph-timeline';

const PAGE_SIZE = 250;
const PAGE_SCAN_LIMIT = 2_000;
const REMOTE_CONNECT_TIMEOUT_MS = 6_000;
const LIVE_REMOTE_READ_TIMEOUT_MS = 30_000;
const HISTORICAL_REMOTE_READ_TIMEOUT_MS = 60_000;
...
class RemoteRuntimeReaderPool {
  ...
  private connect(id: string, signature: string, wsUrl: string, apiKey: string): PendingPoolEntry {
    ...
    void adapter.connect({
      mode: 'remote', runtimeId: id, wsUrl, authKey: apiKey, requestTimeoutMs: this.requestTimeoutMs,
...
const timelineRemoteReaders = new RemoteRuntimeReaderPool(HISTORICAL_REMOTE_READ_TIMEOUT_MS);
const graphRemoteReaders = new RemoteRuntimeReaderPool(LIVE_REMOTE_READ_TIMEOUT_MS);
