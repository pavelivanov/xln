...
const projectTokenMetadata = (
  ...
  const tokenSymbol = typeof metadata?.symbol === 'string' && metadata.symbol.trim() ? metadata.symbol.trim() : null;
  const tokenName = typeof metadata?.name === 'string' && metadata.name.trim() ? metadata.name.trim() : null;
...
const projectPendingPayment = (
  ...
  return {
    recipientEntityId,
    tokenId,
    ...projectTokenMetadata(tokenId, options.resolveTokenMetadata),
    recipientAmount,
    hashlock,
    maxSenderDebit,
    maxFee,
...
export const projectConsensusPayments = (
  ...
  proposal.action.type === 'entity_transaction'
    ? proposal.action.data.txs.flatMap((tx, txIndex) =>
        tx.type === 'htlcPayment' ? [projectPendingPayment(proposal.id, tx, txIndex, options)] : [],
      )
    : [];
