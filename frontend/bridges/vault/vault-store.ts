...
import { closeRuntimeSession } from '../../packages/browser/src/vault/runtimeSession';
import { removePasswordVault } from '../../packages/browser/src/vault/passwordVault';
...
import { writeHubJoinPreference, writeSavedCollateralPolicy } from '../../packages/browser/src/onboarding/onboarding-preferences';
...
import {
  activeSigner,
  clearVaultState,
  loadVaultStateFromStorage,
  persistRuntimeMetadataSnapshot,
  persistVaultStateOrThrow,
  readPersistedVaultProtection,
  runtimesState,
} from './vault-metadata-store';
import {
  DEFAULT_VAULT_UNLOCK_DURATION_MS,
  assertRuntimeAuthorityLease as assertVaultRuntimeAuthorityLease,
  clearVaultLockTimer,
  lockExpiredRuntimeLeases as lockExpiredVaultRuntimeLeases,
  protectRuntimeForDevice as protectVaultRuntimeForDevice,
  restoreRuntimeFromDevice as restoreVaultRuntimeFromDevice,
  scheduleVaultLock as scheduleVaultRuntimeLock,
  unlockVaultRuntimeSecrets as unlockVaultRuntimeAuthority,
} from './vault-authority-lifecycle';
import {
...
export { DEFAULT_VAULT_UNLOCK_DURATION_MS } from './vault-authority-lifecycle';
...
const lockVaultRuntime = (runtimeId: string, expectedProtection?: ProtectedVaultSecrets): Promise<void> =>
  vaultOperations.lockRuntime(runtimeId, expectedProtection);

const scheduleVaultLock = (runtime: Runtime): void => scheduleVaultRuntimeLock(runtime, lockVaultRuntime);

const assertRuntimeAuthorityLease = (runtime: Runtime): void =>
  assertVaultRuntimeAuthorityLease(runtime, lockVaultRuntime);

const protectRuntimeForDevice = (
  runtime: Runtime,
  durationMs: VaultUnlockDurationMs,
  persist: () => void,
): Promise<void> => protectVaultRuntimeForDevice(runtime, durationMs, persist, lockVaultRuntime);

const restoreRuntimeFromDevice = (runtime: Runtime): Promise<boolean> =>
  restoreVaultRuntimeFromDevice(runtime, lockVaultRuntime);
...
): Promise<string> => unlockVaultRuntimeAuthority(runtimeId, seed, durationMs, lockVaultRuntime);
...
export const vaultOperations = {
  ...
  async lockExpiredRuntimeLeases(now = Date.now()): Promise<void> {
    await lockExpiredVaultRuntimeLeases(now, lockVaultRuntime);
  },
  ...
  async lockRuntime(runtimeId: string, expectedProtection?: ProtectedVaultSecrets): Promise<void> {
    ...
    clearVaultLockTimer(normalizedRuntimeId);
    const runtimeEntry = get(runtimes).get(normalizedRuntimeId);
