...
import type { FrontendXlnFunctions } from '../../runtime/xln-store';
...
function pendingBatchIsSelfEntity(value: unknown, selfEntityId: string): boolean {
  const self = String(selfEntityId || '')
    .trim()
    .toLowerCase();
...
export function buildPendingBatchPreview(
  ...
  for (const [index, op] of (batch.settlements || []).entries()) {
    ...
    const reserveLabel =
      reserveDelta > 0n ? 'Settlement (+Reserve)' : reserveDelta < 0n ? 'Settlement (-Reserve)' : 'Settlement';
    pushItem(phase, {
      key: `settle-${index}`,
      title: reserveLabel,
...
export function formatBatchReserveIssue(
  ...
  const tokenLabel = pendingBatchTokenAmountLabel(issue.tokenId, issue.requiredAmount, options)
    .replace(/^[\d.,\s]+/, '')
    .trim();
