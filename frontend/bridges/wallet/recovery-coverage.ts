...
import type {
  ...
  RecoveryCoverageItem,
  RecoveryTowerStatusItem,
} from '../../packages/browser/src/recovery/recovery-coverage-view';
...
const latestPeerFailure = (
  ...
  Array.isArray(failures) ? ([...failures].reverse().find(failure => failure?.source === 'peer') ?? null) : null;
...
export function buildRuntimeRecoveryCoverage(input: RecoveryCoverageInput): RecoveryCoverageItem[] {
  ...
  const receipts = (runtime?.recovery?.lastTowerReceipts || []).filter(receipt =>
    towerUrls.has(normalizeUrl(receipt.towerUrl)),
  );
  ...
  return [
    ...
    },
    {
      ...
      label: 'Tower backup',
      status:
        backupTowerCount === 0 ? 'missing' : backupReceipts.length > 0 || observedBytes > 0 ? 'ready' : 'configured',
      statusLabel:
        backupTowerCount === 0
          ? 'Off'
    ...
    },
    {
      ...
      statusLabel:
        lastResortTowerCount === 0 ? 'Off' : lastResortReceipts.length > 0 ? 'Receipt observed' : 'Configured',
      detail:
        lastResortTowerCount === 0
...
export function buildRecoveryTowerStatuses(
  ...
  return normalizeRecoveryCoverageTowers(towers).map(tower => {
    const url = normalizeUrl(tower.url);
    const latestReceipt =
      receipts.filter(receipt => normalizeUrl(receipt.towerUrl) === url).sort(newestFirst)[0] || null;
    const latestFailure =
      failures.filter(failure => normalizeUrl(failure.towerUrl) === url).sort(newestFirst)[0] || null;
