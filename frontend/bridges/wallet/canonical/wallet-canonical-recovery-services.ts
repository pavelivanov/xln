...
import {
  ...
  type Runtime,
} from '../../vault/recovery/vault-recovery';
...
} from '../../vault/recovery/recovery-settings';
...
const projectRecoveryServices = (runtime: Runtime): WalletRecoveryServicesReadyView => {
  ...
  const discovery = readRuntimeRecoveryDiscoveryStatus(runtime.id);
  ...
  return {
    ...
    discoveryFailures: (discovery?.failures ?? []).map(formatRuntimeRecoveryDiscoveryFailure),
  };
};
